...
function flowplayerJS() {
	$curdir = getcwd();
	chdir(SERVERPATH . '/' . USER_PLUGIN_FOLDER . '/flowplayer3');
	...
	chdir($curdir);
	?>
	<script type="text/javascript" src="<?php echo WEBPATH . '/' . USER_PLUGIN_FOLDER; ?>/flowplayer3/<?php echo $player; ?>"></script>
	<script type="text/javascript" src="<?php echo WEBPATH . '/' . USER_PLUGIN_FOLDER; ?>/flowplayer3/<?php echo $playlist; ?>"></script>
	<?php
}
...
function flowplayer_playlistJS() {
	...
	if (file_exists($css)) {
		...
		$css = WEBPATH . '/' . USER_PLUGIN_FOLDER . '/flowplayer3/flowplayer3_playlist.css';
	}
	?>
	<script type="text/javascript" src="<?php echo WEBPATH . '/' . USER_PLUGIN_FOLDER; ?>/flowplayer3/jquery.tools.min.js"></script>
	<link rel="stylesheet" type="text/css" href="<?php echo pathurlencode($css); ?>" />
	<?php
}

if (!defined('FLOW_PLAYER_MP3_HEIGHT'))
	define('FLOW_PLAYER_MP3_HEIGHT', 26);
...
class Flowplayer3 {
	...
	function getPlayerConfig($moviepath, $imagetitle = '', $count = '', $width = NULL, $height = NULL) {
		...
		// <!-- <![CDATA[
		flowplayer("player' . $count . '","' . WEBPATH . '/' . USER_PLUGIN_FOLDER . '/flowplayer3/' . $swf . '", {
		plugins: {
			audio: {
				url: "' . $audio . '"
			},
...
function flowplayerPlaylist($option = "playlist", $albumfolder = "") {
	...
	switch ($option) {
		...
		case 'playlist-mp3':
			...
			flowplayer("player' . $album->getID() . '","' . WEBPATH . '/' . USER_PLUGIN_FOLDER . '/flowplayer3/' . $swf . '", {
			plugins: {
				audio: {
					url: "' . $audio . '"
				},
