...
$plugin_notice = sprintf(gettext('Run the <a href="%s">OpenID detect</a> script to check compatibility of your server configuration.'), FULLWEBPATH . '/' . USER_PLUGIN_FOLDER . '/federated_logon/Auth/OpenID_detect.php?test_query=a%26b');
$plugin_author = "Stephen Billard (sbillard)";
$plugin_category = gettext('Users');

$plugin_disable = (getOption('federated_logon_detect')) ? false : sprintf(gettext('The <a href="%s">OpenID detect</a> script has not been run.'), FULLWEBPATH . '/' . USER_PLUGIN_FOLDER . '/federated_logon/Auth/OpenID_detect.php?test_query=a%26b');
if ($plugin_disable) {
...
class federated_logon {
	...
	function getOptionsSupported() {
		...
		$options = array(
				gettext('Assign user to') => array(
						'key' => 'federated_login_group',
						'type' => OPTION_TYPE_SELECTOR,
						'order' => 0,
						'selections' => $ordered,
						'desc' => gettext('The user group to which to map the federated login.')),
				gettext('Handlers') => array(
			...
			foreach ($alt_handlers as $handler => $details) {
				...
				<li>
					<span class="fed_buttons">
						<button type="button" onclick="window.location = '<?php echo $script; ?>?<?php echo $params; ?>" title="<?php echo $authority; ?>">
							<?php
