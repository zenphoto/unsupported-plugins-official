...
class tweet {
	...
	function getOptionsSupported() {
		global $_zp_zenpage;
		$options = array(gettext('Consumer key') => array(
						'key' => 'tweet_news_consumer',
				...
				gettext('Secret') => array(
						'key' => 'tweet_news_consumer_secret',
						'type' => OPTION_TYPE_TEXTBOX,
						'order' => 3,
						'desc' => gettext('The <em>secret</em> associated with your <em>consumer key</em>.')),
				gettext('Access token') => array(
						'key' => 'tweet_news_oauth_token',
						'type' => OPTION_TYPE_TEXTBOX,
						'order' => 4,
						'desc' => gettext('The application <em>oauth_token</em> token.')),
				gettext('Access token secret') => array(
						'key' => 'tweet_news_oauth_token_secret',
						'type' => OPTION_TYPE_TEXTBOX,
						'order' => 5,
						'desc' => gettext('The application <em>oauth_token</em> secret.')),
				gettext('Protected objects') => array(
						'key' => 'tweet_news_protected',
		...
		if (getOption('tweet_news_news') && is_object($_zp_zenpage)) {
			...
			$options[gettext('News categories')] = array(
					'key' => 'tweet_news_categories',
	...
	private static function tweetObject($obj) {
		...
		if (class_exists('tinyURL')) {
			$link = SERVER_HTTP_HOST . tinyURL::getURL($obj);
		} else {
			$link = SERVER_HTTP_HOST . $obj->getLink();
		}
		switch ($type = $obj->table) {
			case 'pages':
			case 'news':
