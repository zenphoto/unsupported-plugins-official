...
class externalFeed_options {
	...
	function getOptionsSupported() {
		$options = array(gettext('externalFeed feeds enabled:') => array(
						'key' => 'externalFeed_feed_list',
				...
				gettext('Album feed sort order:') => array(
						'key' => 'externalFeed_sortorder_albums',
						'type' => OPTION_TYPE_SELECTOR,
						'selections' => array(gettext('latest by id') => 'latest',
								gettext('latest by date') => 'latest-date',
								gettext('latest by mtime') => 'latest-mtime',
				...
				gettext('New requestor:') => array(
						'key' => 'externalFeed_site',
						'type' => OPTION_TYPE_TEXTBOX,
						'order' => 9,
						'desc' => gettext("Supply a site name to add a new using site.")),
				gettext('Registered sites:') => array(
						'key' => 'externalFeed_sitelist',
		...
		if (extensionEnabled('zenpage')) {
			$options[gettext('Feed text length')] = array(
					'key' => 'externalFeed_truncate_length',
					'type' => OPTION_TYPE_TEXTBOX,
					'order' => 6,
					'desc' => gettext("The text length of the Zenpage feed items. No value for full length."));
			$options[gettext('News feed items')] = array(
					'key' => 'externalFeed_zenpage_items',
					'type' => OPTION_TYPE_TEXTBOX,
					'order' => 5,
