...
class slideshow {
	...
	function getOptionsSupported() {
		$options = array(gettext('Mode') => array(
						'key' => 'slideshow_mode',
		...
		switch (getOption('slideshow_mode')) {
				...
				$options = array_merge($options, array(
						...
						gettext('Cycle Effect') => array(
								...
								'selections' => array(
										...
										gettext('zoom') => "zoom",
										gettext('slide X') => "slideX",
										gettext('slide Y') => "slideY",
										gettext('scroll up') => "scrollUp",
										gettext('scroll down') => "scrollDown",
										gettext('scroll left') => "scrollLeft",
										gettext('scroll right') => "scrollRight"),
								'desc' => gettext("The cycle slide effect to be used.")),
						gettext('Timeout') => array(
			...
			case 'colorbox':
				$options = array_merge($options, array(
						gettext('Colorbox transition') => array(
								'key' => 'slideshow_colorbox_transition',
								'type' => OPTION_TYPE_SELECTOR,
								'order' => 2,
								'selections' => array(
						...
						gettext('Colorbox image type') => array(
								'key' => 'slideshow_colorbox_imagetype',
								'type' => OPTION_TYPE_SELECTOR,
								'order' => 3,
								'selections' => array(
						...
						gettext('Colorbox image title') => array(
								'key' => 'slideshow_colorbox_imagetitle',
								'type' => OPTION_TYPE_CHECKBOX,
								'order' => 4,
	...
	static function header_js() {
		...
		if (file_exists($css)) {
			...
			$css = WEBPATH . '/' . USER_PLUGIN_FOLDER . '/slideshow/slideshow.css';
		}
		?>
		<script	src="<?php echo FULLWEBPATH . '/' . USER_PLUGIN_FOLDER; ?>/slideshow/jquery.cycle.all.js" type="text/javascript"></script>
